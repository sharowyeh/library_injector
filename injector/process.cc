...
namespace injector
{
	BOOL enable_debug_privilege(HANDLE proc_handle, TOKEN_PRIVILEGES *prev_tp_ptr)
	{
	...
	BOOL disable_debug_privilege(HANDLE proc_handle, TOKEN_PRIVILEGES prev_tp)
	{
		HANDLE token_handle = NULL;
		BOOL result = OpenProcessToken(proc_handle, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &token_handle);
		if (result == FALSE)
	...
	BOOL inject_remote_thread(HANDLE proc_handle, LPVOID func_ptr, PBYTE param_ptr, SIZE_T param_size,
		ULONG_PTR mem_begin, ULONG_PTR mem_end, ULONG_PTR mem_align)
	{
		LOGFILE("RemoteThreadMemory: begin=0x%p end=0x%p align=0x%p\n", mem_begin, mem_end, mem_align);
		...
		LPVOID alloc_ptr = VirtualAllocEx(proc_handle, NULL, alloc_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		LOGFILE("VirtualAllocEx: use=DEFAULT alloc_ptr=0x%p size=%d err=%d\n", alloc_ptr, alloc_size, GetLastError());
		if (alloc_ptr == NULL && free_ptr != 0)
		{
			alloc_size = 4096;
			alloc_ptr = VirtualAllocEx(proc_handle, (LPVOID)free_ptr, alloc_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
			LOGFILE("VirtualAllocEx: use=FREE alloc_ptr=0x%p size=%d err=%d\n", alloc_ptr, alloc_size, GetLastError());
		}
		if (alloc_ptr == NULL && reserve_ptr != 0)
		{
			alloc_size = 4096;
			alloc_ptr = VirtualAllocEx(proc_handle, (LPVOID)reserve_ptr, alloc_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		...
		ZeroMemory(&mem_info, sizeof(mem_info));
		info_size = VirtualQueryEx(proc_handle, alloc_ptr, (MEMORY_BASIC_INFORMATION *)(&mem_info), sizeof(mem_info));
		...
		LOGFILE("WriteProcessMemory: result=%d proc=0x%p param=%s size=%d err=%d\n", result, proc_handle, param_ptr, param_size, GetLastError());
		if (result == FALSE)
		{
		...
		HANDLE remote_handle = CreateRemoteThread(proc_handle, NULL, 0, (LPTHREAD_START_ROUTINE)func_ptr, alloc_ptr, 0, NULL);
		LOGFILE("CreateRemoteThread: remote_handle=0x%p err=%d\n", remote_handle, GetLastError());
		...
		WaitForSingleObject(remote_handle, INFINITE);
		DWORD exit_code = 0;
		GetExitCodeThread(remote_handle, &exit_code);
		LOGFILE("ThreadExitCode: 0x%x\n", exit_code);
	...
	BOOL inject_x64_remote_thread(HANDLE proc_handle, LPVOID func_ptr, PBYTE param_ptr, SIZE_T param_size)
	{
		...
		BOOL result = inject_remote_thread(proc_handle, func_ptr, param_ptr, param_size,
			begin, end, align);
		return result;
	}

	BOOL inject_x86_remote_thread(HANDLE proc_handle, LPVOID func_ptr, PBYTE param_ptr, SIZE_T param_size)
	{
	...
	BOOL inject_process_handle(HANDLE proc_handle)
	{
		...
		result = get_machine_type(proc_path, &machine_type);
		LOGFILE("GetFileMachineType: result=%d type=0x%x(0x014C=x86,0x8664=x64)\n", result, machine_type);

		char dll_path_64[MAX_PATH];
		char dll_path_32[MAX_PATH];
		get_this_module_path(dll_path_64, dll_path_32);
		LOGFILE("InjectFilePath: x64=%s\n", dll_path_64);
		LOGFILE("InjectFilePath: x86=%s\n", dll_path_32);

		// If caller process platform architecture is different with target process,
		// It must be use another process to get LoadLibrary method address
		// TODO: or https://github.com/juntalis/project52a/blob/master/injector/src/injdll32_64.c
		// TODO: or https://github.com/rootm0s/Injectors/blob/master/injectAllTheThings/injectAllTheThings/GetProcAddressR.c
		// Get function address
		LPVOID func_ptr = NULL;
		if (machine_type == IMAGE_FILE_MACHINE_AMD64)
		{
#if _WIN64
			func_ptr = get_func_address("kernel32.dll", "LoadLibraryA");
#else
			// not support
			return FALSE;
#endif
			result = inject_x64_remote_thread(proc_handle, func_ptr, (PBYTE)dll_path_64, strlen(dll_path_64));
		}
		else if (machine_type == IMAGE_FILE_MACHINE_I386)
		{
#if _WIN64
			func_ptr = get_func_address(proc_handle, "c:\\windows\\syswow64\\kernel32.dll", "LoadLibraryA");
#else
			func_ptr = get_func_address("kernel32.dll", "LoadLibraryA");
#endif
			result = inject_x86_remote_thread(proc_handle, func_ptr, (PBYTE)dll_path_32, strlen(dll_path_32));
		}
		else
		{
			return FALSE;
		}
		LOGFILE("InjectResult: %d\n", result);
		return result;
	}

	BOOL inject_process_id(DWORD proc_id)
	{
		SetLastError(0);
		HANDLE proc_handle = OpenProcess(
			...
			PROCESS_VM_READ,
			FALSE, proc_id);
		LOGFILE("OpenProcess: id=%d handle=0x%p err=%d\n", proc_id, proc_handle, GetLastError());
		if (proc_handle == NULL)
			return FALSE;
		...
		BOOL token_result = enable_debug_privilege(proc_handle, &prev_tp);
		LOGFILE("EnableDebugPrivilege: result=%d\n", token_result);

		BOOL result = inject_process_handle(proc_handle);
		LOGFILE("InjectProcessHandle: result=%d\n", result);

		if (token_result == TRUE)
			disable_debug_privilege(proc_handle, prev_tp);
